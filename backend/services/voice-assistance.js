...
export const processCommand = ({ command, username }) => {
  ...
  if (normalizedCommand.includes("hey") || normalizedCommand.includes("hello")) {
    ...
    action = "open";
  } else if (normalizedCommand.includes("search book")) {
    const bookName = command.replace("search book", "").trim();
    console.log("Searching for book:", bookName);

    if (bookName) {
      action = "searchBook"; // Custom action for triggering search in Header.jsx
      responseText = `Searching for book: ${bookName}`;
      url = `/books?query=${encodeURIComponent(bookName)}`;
    } else {
      responseText = "Please specify the book name to search.";
    }
    ...
    action = "open";
    url = "Calculator:///";
  } else if (normalizedCommand.includes("open book page") || normalizedCommand.includes("open book") || normalizedCommand.includes("book page")) {
    responseText = "Opening book page";
    action = "navigate";
    url = "/user-books";
