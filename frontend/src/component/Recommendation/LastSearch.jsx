...
const LastSearches = () => {
  const [lastTwoBooks, setLastTwoBooks] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  const email = localStorage.getItem("email");

  // Fetch the last two book searches
  useEffect(() => {
    const fetchLastTwoBooks = async () => {
      try {
        ...
        if (response.data && response.data.recommendations) {
          setLastTwoBooks(response.data.recommendations);
        } else {
  ...
  if (loading) return <div>Loading last two book searches...</div>;
  ...
  return (
    <div style={containerStyle}>
      ...
      {lastTwoBooks && lastTwoBooks.length > 0 ? (
        ...
        <div>No books found from the last two searches.</div>
      )}
    </div>
  );
};
