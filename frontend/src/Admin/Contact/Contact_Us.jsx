...
const Contact_Us = () => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:5000/api/save-contact", {
        ...
        body: JSON.stringify(formDataWithoutReply), // Send without the reply field
      });
  ...
  const sendEmail = async (formData) => {
    const emailTemplateParams = {
      from_name: 'Book Haven Admin',        
      from_email: 'admin@bookhaven.com',   
      to_name: formData.name,              
      to_email: formData.email,            
      message: formData.message,           
      reply: formData.reply,               
      store_name: 'Book Haven',            
      reply_to: 'admin@bookhaven.com'      
    };
